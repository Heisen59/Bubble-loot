...
-- Helping function 
-- Function to check if a player is a guild officer
function IsGuildOfficer(playerName)
    -- Update the guild roster information
    GuildRoster()

    -- Loop through all guild members
    for i = 1, GetNumGuildMembers() do
        -- Get information about each guild member
        local name, rank, rankIndex = GetGuildRosterInfo(i)
        
        -- Normalize the playerName and guild member name for comparison
        if name and string.lower(name) == string.lower(playerName) then
            -- Officer rankIndex is usually 1 or 2 (depending on the guild rank settings)
            -- Adjust this condition based on how your guild ranks are organized.
            if rankIndex == 1 or rankIndex == 2 then
                return true -- Player is an officer
            end
        end
    end
    return false -- Player is not an officer
end
...
function BubbleLoot_G.sync.BroadcastDataTable(msgType, tbl, targetPlayer)
    ...
    if targetPlayer then
        ...
        for _, trustedPlayer in ipairs(SyncTrustList[1]) do            
            C_ChatInfo.SendAddonMessage(prefix, serializedData, "WHISPER", trustedPlayer)
        end
...
local function registerNewData(msgType, receivedTable)
	...
	elseif msgType == cfg.SYNC_MSG.ADD_PLAYER_DATA_FUNCTION then
		BubbleLoot_G.storage.AddPlayerData(receivedTable, true)
	else
...
local function checkSender(sender, msgType, receivedTable)
    ...
    -- discard if not guild officer or in white list
